...
recquirements_dict['k']=1.5   # Let us define k=1+gds*Rd
...
header=['n','Io','W','Rb','Rd','fo','iip3','gain','s11','nf','PWR','gm','CF','CF_wo_Io']
writer.writerow(header)




#hand_out_dict['Io']=400e-6
#-------------------Optimisation-------------------------------
...
sol=[]
CF,CF_wo_Io,out_check=optimize.cost_function(recquirements_dict,output_dict,hand_out_dict)
print('Cost function=',CF)
count=1
best_iter=-1
flag2=0
lf=20e-2 		# Common Learning Factor 
...
for i in range(250):
	...
	coeff_grad,coeff_lf=optimize.Gradient(CF,CF_wo_Io,out_check,hand_out_dict,recquirements_dict,extract_param,cir_filename,chi_filename)
	#print(hand_out_dict)
	...
	if abs(stop_cond)<0.001:					# If Avg Slope of Io is less than 1uA/Iteration it will break
		print("Stop Reached")		
		#break
	

	writer.writerow([count,hand_out_dict['Io'],hand_out_dict['W'],hand_out_dict['Rb'],hand_out_dict['Rd'],hand_out_dict['fo'],output_dict['iip3_dBm'],output_dict['gain_dB'],output_dict['s11_dB'],output_dict['NF_dB'],output_dict['pwr_dc'],extract_param['gm1'],CF,CF_wo_Io])
...
py.clf()
fig9=py.plot(CF_wo_Io_list)
py.savefig("CF_wo_Io.png")
py.clf()
fig9=py.plot(W_list)
py.savefig("W.png")
py.clf()
