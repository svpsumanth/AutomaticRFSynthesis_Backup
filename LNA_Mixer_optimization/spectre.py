...
def extract_dc_param(optimization_input_parameters):
	...
	extracted_parameters['Io1']=valueE_to_value(lines[8])
	extracted_parameters['Io2']=valueE_to_value(lines[16])
	extracted_parameters['gm1']=valueE_to_value(lines[9])
	extracted_parameters['gds1']=valueE_to_value(lines[10])
	extracted_parameters['vth1']=valueE_to_value(lines[12])	
	extracted_parameters['vdsat1']=valueE_to_value(lines[13])
	extracted_parameters['cgs1']=abs(valueE_to_value(lines[14]))	
	extracted_parameters['cgd1']=abs(valueE_to_value(lines[15]))
	extracted_parameters['region1']=np.absolute(valueE_to_value(lines[11]))
	extracted_parameters['gm2']=valueE_to_value(lines[17])
	extracted_parameters['gds2']=valueE_to_value(lines[18])
	extracted_parameters['vth2']=valueE_to_value(lines[20])	
	extracted_parameters['vdsat2']=valueE_to_value(lines[21])
	extracted_parameters['region2']=np.absolute(valueE_to_value(lines[19]))
	extracted_parameters['cgs2']=abs(valueE_to_value(lines[22]))	
	extracted_parameters['cgd2']=abs(valueE_to_value(lines[23]))
	extracted_parameters['vth3']=valueE_to_value(lines[27])	
	extracted_parameters['cgs3']=abs(valueE_to_value(lines[29]))	
	extracted_parameters['cgd3']=abs(valueE_to_value(lines[30]))	
	extracted_parameters['Vcmlo']=valueE_to_value(lines[5])
	extracted_parameters['vd3']=valueE_to_value(lines[4])
	extracted_parameters['vg2']=valueE_to_value(lines[7])
...
def extract_sp_param(optimization_input_parameters):
	...
	#lines=lines[26:]	#4ports
	lines=lines[20:]	#3ports
	line1=lines[0].split()
	

	s11_db=float(line1[1].split(',')[0])	
	s11_rad=float(line1[2])

	extracted_parameters['s11_db']=s11_db
	extracted_parameters['s11_rad']=s11_rad
	return extracted_parameters
#---------------------------------------------------------------------------------------------------------------------------	
...
def extract_conv_gain(optimization_input_parameters,Vinp='S2p',Vinn='S2n',Voutp='Voutp',Voutn='Voutn'):
	...
	for line in lines:
		...
		if freq_flag==1:
			...
			if "\""+"D2p"+"\"" in line:
				Vlnap_list.append(extract_complex_vout(line))

			if "\""+"D2n"+"\"" in line:
				Vlnan_list.append(extract_complex_vout(line))
	...
	Vlnap_list=np.array(Vlnap_list)
	Vlnan_list=np.array(Vlnan_list)
	Vlna_list=Vlnap_list-Vlnan_list
	rf_gain=Vlna_list/Vin_list

	lna_gain=abs(rf_gain[0])
	extracted_parameters['lna_gain_db']=20*np.log10(lna_gain)
	extracted_parameters['lna_gain_freq']=freq_in_list[0]
...
def extract_hb_magnitude(file_name,freq,node="Vout"):
	...
	for line in lines:
		...
		if freq_match_flag==1:
			if node in line:
				flag_node_extract=1
				vol=extract_complex_vout(line)
		
		if flag_node_extract==1:
			break
...
def extract_iip3_manual_sweep_diff(file_name,optimization_input_parameters,Vinp='S2p',Vinn='S2n',Voutp='Voutp',Voutn='Voutn'):
...
def run_file(optimization_input_parameters):
	...
	p1=Process(target=run_file_tcsh,args=(optimization_input_parameters['filename']['tcsh_run'],))
	p1.start()
	p2=Process(target=run_file_iip3_tcsh,args=(optimization_input_parameters['filename']['tcsh_iip3_run'],))
	p2.start()
	p1.join()
	p2.join()
